...
def check_credits(credits, subjects):
...
def admit_candidate(name, jamb_score, credits, subjects, department, interview):
    if check_admission_requirements(jamb_score, department) and check_credits(credits, subjects) and interview:
    ...
    while True:
        ...
        if choice == "1":
            ...
            for i in range(num_credits):
                ...
                credits.append(grade)
                subjects.append(subject)
            interview = input("Has the candidate passed the interview? (yes/no): ")
            interview = interview.lower() == "yes"
            admit_candidate(name, jamb_score, credits, subjects, department, interview)
        elif choice == "2":
            print("Admitted Candidates:")
            for candidate in admitted:
                print(f"Name: {candidate[0]}, Department: {candidate[1]}")
        elif choice == "3":
